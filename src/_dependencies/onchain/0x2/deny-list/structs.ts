...
export function isDenyList(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::deny_list::DenyList`; }
...
export class DenyList implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V1}::deny_list::DenyList`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = DenyList.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::deny_list::DenyList`; readonly $typeArgs: []; readonly $isPhantom = DenyList.$isPhantom;

 readonly id: ToField<UID>; readonly lists: ToField<Bag>

 private constructor(typeArgs: [], fields: DenyListFields, ) { this.$fullTypeName = composeSuiType( DenyList.$typeName, ...typeArgs ) as `${typeof PKG_V1}::deny_list::DenyList`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.lists = fields.lists; }

 static reified( ): DenyListReified { return { typeName: DenyList.$typeName, fullTypeName: composeSuiType( DenyList.$typeName, ...[] ) as `${typeof PKG_V1}::deny_list::DenyList`, typeArgs: [ ] as [], isPhantom: DenyList.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => DenyList.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => DenyList.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => DenyList.fromBcs( data, ), bcs: DenyList.bcs, fromJSONField: (field: any) => DenyList.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => DenyList.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => DenyList.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => DenyList.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => DenyList.fetch( client, id, ), new: ( fields: DenyListFields, ) => { return new DenyList( [], fields ) }, kind: "StructClassReified", } }
...
export function isConfigWriteCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::deny_list::ConfigWriteCap`; }
...
export function isConfigKey(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::deny_list::ConfigKey`; }
...
export function isAddressKey(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::deny_list::AddressKey`; }
...
export function isGlobalPauseKey(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::deny_list::GlobalPauseKey`; }
...
export function isPerTypeConfigCreated(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::deny_list::PerTypeConfigCreated`; }
...
export function isPerTypeList(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::deny_list::PerTypeList`; }
...
export class PerTypeList implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V1}::deny_list::PerTypeList`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = PerTypeList.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::deny_list::PerTypeList`; readonly $typeArgs: []; readonly $isPhantom = PerTypeList.$isPhantom;
